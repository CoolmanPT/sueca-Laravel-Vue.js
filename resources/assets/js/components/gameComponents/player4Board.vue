<template>
    <div>
        ...
        <div>
            ...
            <div v-if="game.players[3].playerID != game.playerTurn" v-for="(player, index) in game.players">
                <h3 v-if="player.playerID === game.playerTurn" class="text-center bg-info text-white">{{player.name}} Turn</h3>
            </div>
            <div v-if="game.players[3].playerID == game.playerTurn">
...
export default {
  ...
  computed: {
    isDisabled: function() {
      return this.game.playerTurn != this.game.players[3].playerID;
    },
    message: function() {
